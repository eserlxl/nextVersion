...
# - Supports --complexity (more files, longer files, more history)
...
normalize_complexity() { # [complexity] map aliases and clamp
  local x="${1:-5}"
  case "$x" in
    low)  x=2 ;;
    med|medium) x=5 ;;
...
[[ -x "$ANALYZER_SH" ]] || { fail "Analyzer not found/executable: $ANALYZER_SH"; exit 1; }
[[ -x "$NEXT_VERSION_BIN" ]] || { fail "Binary not found/executable: $NEXT_VERSION_BIN"; exit 1; }
...
rand_pick() { local -a items=("$@"); local idx=$(( RANDOM % ${#items[@]} )); echo "${items[$idx]}"; }
...
git_set_identity() { git -c init.defaultBranch=main init -q; git config user.name  "NV Test $(rand_word)"; git config user.email "nv-$(rand_word)@example.com"; }
...
rename_random_file() {
  ...
  [[ -e "$new" ]] && new="${dir}/renamed_$(rand_word)_${RANDOM}_${base}"
...
generate_random_repos() { local count="${1:-10}"; local i=0; while (( i < count )); do generate_random_repo; ((++i)); done; }
...
main() {
  ...
  if (( tests_failed == 0 )); then
    info "All repository comparisons PASSED"; exit 0
  else
    warn "Some repository comparisons FAILED"; exit 1
  fi
}
